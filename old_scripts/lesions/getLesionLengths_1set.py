        ...
        for ii in range(new_volume.shape[0]):
            for jj in range(new_volume.shape[1]):
                for kk in range(new_volume.shape[2]):
                    if (new_volume[ii, jj, kk] != 0):
                        new_volume[ii, jj, kk] = 1
...
val_list = [1] # list of values used to label ROI types.
...
#    label_val = np.amax(volume)
    ...
    for lesion_index in range(num_lesions):
        ...
        coords = lesions[lesion_index]
        ave_label = 0
        for coord in coords:
            label = orig_volume[coord[0], coord[1], coord[2]]
            ave_label += label
        ave_label = float(ave_label)/float(len(coords))
        print "Lesion label         : "+str(ave_label)
...
#print id, visit, len_max_right, vol_max_right, num_right, vol_tot_right
