...
# 2020-03-06 -  Steven  Ufkes:  Add path to modified scripts.
ANTSPATHALT=/hpf/largeprojects/smiller/tools/ants_modified
...
PBS=${ANTSPATHALT}/waitForPBSQJobs.pl
  ...
  then
    ...
    for (( i = 0; i < ${#IMAGESETARRAY[@]}; i+=$NUMBEROFMODALITIES ))
      do
        ...
        if [[ $DOQSUB -eq 1 ]];
          ...
          then
            id=`qsub -N antsrigid -v ANTSPATH=$ANTSPATH $QSUBOPTS -l nodes=1:ppn=1 -l mem=${MEMORY},vmem=${MEMORY} -l walltime=${WALLTIME} $qscript | awk '{print $1}'`
            jobIDs="$jobIDs $id"
            sleep 0.5
        elif [[ $DOQSUB -eq 2 ]];
          then
    ...
    if [[ $DOQSUB -eq 4 ]];
      then
        ...
        echo "--------------------------------------------------------------------------------------"
               # now wait for the jobs to finish. Rigid registration is quick, so poll queue every 60 seconds
        ${ANTSPATHALT}/waitForPBSQJobs.pl 1 60 $jobIDs
        # Returns 1 if there are errors
        if [[ ! $? -eq 0 ]];
          then
            echo "qsub submission failed - jobs went into error state"
  ...
  do
      ...
      do
        ...
        if [[ $DOQSUB -eq 1 ]];
          ...
          then
            ...
            echo -e "$exe" >> $qscript
            id=`qsub -N antsdef${i} -v ANTSPATH=$ANTSPATH -l nodes=1:ppn=1 -l mem=${MEMORY},vmem=${MEMORY} -l walltime=${WALLTIME} $QSUBOPTS $qscript | awk '{print $1}'`
            jobIDs="$jobIDs $id"
            sleep 0.5
        elif [[ $DOQSUB -eq 2 ]];
          then
    ...
    if [[ $DOQSUB -eq 1 ]];
      ...
      then
        ...
        echo "--------------------------------------------------------------------------------------"
        # now wait for the stuff to finish - this will take a while so poll queue every 10 mins
	# Steven Ufkes - 2020-06-04: I changed this to poll every 2 minutes.
        ${ANTSPATHALT}/waitForPBSQJobs.pl 1 120 $jobIDs
        if [[ ! $? -eq 0 ]];
          then
            echo "qsub submission failed - jobs went into error state"
            exit 1;
