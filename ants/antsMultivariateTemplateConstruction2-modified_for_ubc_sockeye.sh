...
#>>>>
allocation_name='st-smiller6-1' # allocation name for running on UBC Sockeye cluster.
#<<<<
...
if [[ ${NINFILES} -eq 0 ]];
    ...
    #>>>> # Get the right extension even if the input file name contains dots.
    #extension=`echo ${IMAGESETVARIABLE#*.}`
    extension=$(echo "$IMAGESETVARIABLE" | rev | cut -d '.' -f 1 | rev)
    #<<<<
  ...
  then
    ...
    for (( i = 0; i < ${#IMAGESETARRAY[@]}; i+=$NUMBEROFMODALITIES ))
      do
	    ...
	    id=`qsub -A "$allocation_name" -N antsrigid -v ANTSPATH=$ANTSPATH -l select=1:ncpus=1:mem=${MEMORY} -l walltime=${WALLTIME} $qscript | awk '{print $1}' | cut -d '.' -f 1`
	    #<<<<
            jobIDs="$jobIDs $id"
            sleep 0.5
        elif [[ $DOQSUB -eq 2 ]];
  ...
  do
    ...
    for (( j = 0; j < ${#IMAGESETARRAY[@]}; j+=$NUMBEROFMODALITIES ))
      do
	    ...
	    id=`qsub -A "$allocation_name" -N antsdef${i} -v ANTSPATH=$ANTSPATH -l select=1:ncpus=1:mem=${MEMORY} -l walltime=${WALLTIME} $qscript | awk '{print $1}' | cut -d '.' -f 1`
	    #<<<<
