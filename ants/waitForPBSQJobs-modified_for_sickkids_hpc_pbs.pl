...
while( $jobsIncomplete )
  ...
  foreach my $job (@jobIDs)
	...
	# Check status - there's no error state in PBS
        ...
        # 2020-03-06 - Steven Ufkes hack: check if job is in state 'C' (complete), if so consider job complete.
        if( !($tokens[$statePos] =~ m/C/) )
         {
          $jobsIncomplete = $jobsIncomplete + 1;
	  }
#          $jobsIncomplete = $jobsIncomplete + 1;
#     	The line above was replaced with the hack.
#     	END OF HACK
        if( $verbose )
          {
